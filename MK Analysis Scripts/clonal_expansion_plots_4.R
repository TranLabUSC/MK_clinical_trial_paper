...
patient_metadata_df$group <- ifelse(
  ...
  ifelse(
    ...
    ifelse(
      ...
      NA_character_  # or "unknown" if you prefer
    )
  )
)
...
######################################################################################################################################################################################################################
# aggregated pdf with all arm_types
library(Seurat)
library(dplyr)
library(data.table)
library(ggplot2)
...
#-----------------------------------------------------------
# New function to create a list of ggplots for each arm, 
# and then combine them into a single PDF.
#-----------------------------------------------------------
plot_comparison_all_arms <- function(df_merged,
  ...
  # Filter out rows that are NA for our variable of interest
  df_merged <- df_merged[!is.na(df_merged[[variable_name]]), ]
  
  # Subset for the control arm
  control_df <- df_merged[df_merged$group == control_arm, ]
  
  # Determine y-axis limits across ALL arms (to keep y-scale consistent)
  y_min <- min(df_merged[[variable_name]], na.rm = TRUE)
  y_max <- max(df_merged[[variable_name]], na.rm = TRUE)
  
  # We’ll store plots for each arm in a list
  plot_list <- list()
  ...
  for (arm in unique_arms) {
    # Subset the data for the current arm
    df_arm <- df_merged[df_merged$group == arm, ]
    finite_values <- df_arm[[variable_name]][is.finite(df_arm[[variable_name]])]
    
    # Calculate p-value comparing to control arm (except for control arm itself)
    p_val <- NA
    p_label <- ""
    col <- "black"
    ...
    p <- ggplot(df_arm, aes(x = arm, y = !!sym(variable_name))) +
      ...
      geom_boxplot(width = 0.1, outlier.shape = NA) +   # Remove outlier dots
      geom_point(position = position_jitter(width = 0.1)) +
      scale_y_continuous(name = variable_name, limits = c(y_min, y_max)) +
      ylab("Clonal Diversity Ratio") +
      ggtitle(plot_title) +
      geom_hline(yintercept = 1, linetype = "dotted", color = "red", linewidth = 1) +  # Red dotted line at y=1
      theme_bw() +
      theme(
        legend.position = "none",
        text = element_text(size = 10),
        ...
        panel.border = element_blank(),
        axis.line = element_line(colour = "black"),
        # Remove x-axis label, ticks, and tick labels
        axis.title.x = element_blank(),
        axis.text.x  = element_blank(),
  ...
  # Combine all arm plots into a grid
...
#-----------------------------------------------------------
# Main loop over celltypes and diversity indices
#-----------------------------------------------------------
...
for (celltype in celltypes) {
  ...
  for (diversity_index in diversity_indices) {
    ...
    # Build your output directory for this (celltype, diversity_index)
    plot_dir <- paste0(dir, "/clonal_expansion_", diversity_index, "_", celltype, "_T_cells_division/")
    ...
    df_merged <- merge(df, 
                       ...
                       all.x = TRUE, 
                       all.y = FALSE)
    colnames(df_merged)[1] <- "patient_id"  # rename merged column
    ...
    # Generate a single PDF with plots for all arms
    # You can change 'control_arm' if your control is named differently
...
# April 22
# ---------------------------------------------------------------------------
#  Clonal diversity paired‑boxplots across timepoints & patient cohorts
#  Author: ChatGPT (April 22 2025)
#  Description: Variant of previous clonal‑expansion script that
#  • reads *actual* clonotype diversity (Shannon / Simpson)
#  • for every T‑cell sub‑population and every time‑point pair
#    (Pre vs C1, Pre vs C2, C1 vs C2)
#  • produces paired boxplots (one box per timepoint, dots = patients,
#    lines connect paired observations) faceted by clinical cohort
#  • performs paired t‑test inside each cohort and annotates p‑value
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 1. Patient metadata & cohort assignment
# ---------------------------------------------------------------------------
patient_metadata_file <- "/project/dtran642_927/Data/Collaborator/DJ/scRNAseq/latest_big_sequencing_projects/MK/Analysis/UF_WUSTL_combined/Aggregated/outs/NETZEN_analysis/new_analysis/patient_survival_data.csv"
patient_metadata_df  <- fread(patient_metadata_file) %>%
  filter(site == "UF") %>%
  mutate(patient_id = as.integer(patient_id))  # <‑‑ ensure integer type to match parsed IDs
...
# Cohort‑specific light/dark palettes ---------------------------------------
cohort_shades <- list(
  control    = c("#FFF8B0", "#FFD700"),  # light yellow / gold
  short_term = c("#AED7FF", "#1E90FF"),  # light blue / dodgerblue
  long_term  = c("#FFAD99", "#FF4500")   # light salmon / orangered
)

cohort_order <- c("control", "short_term", "long_term")

# ---------------------------------------------------------------------------
# 2. Cell‑type / cluster mapping (unchanged from original script)
# ---------------------------------------------------------------------------
...
parse_diversity_file <- function(fp) {
  fread(fp, header = FALSE, sep = "\t", col.names = c("sample_id", "diversity")) %>%
    filter(grepl("^[0-9]", sample_id)) %>%
    mutate(patient_id = as.integer(gsub("^([0-9]+).*", "\\1", sample_id)),
           tp_code    = gsub("^[0-9]+", "", sample_id))
}

plot_pair_for_cohort <- function(df_pair, cohort_name, tp_codes, y_lim) {
  df_arm <- df_pair %>%
    filter(group == cohort_name) %>%
    group_by(patient_id) %>% filter(n_distinct(tp_code) == 2) %>% ungroup()
  if (nrow(df_arm) == 0) return(NULL)
  
  # paired t‑test
  wide   <- df_arm %>% select(patient_id, tp_code, diversity) %>%
    pivot_wider(names_from = tp_code, values_from = diversity) %>% na.omit()
  p_val  <- t.test(wide[[tp_codes[1]]], wide[[tp_codes[2]]], paired = TRUE)$p.value
  p_lab  <- sprintf("paired t p = %.3g%s", p_val, ifelse(p_val <= 0.05, " *", ""))
  
  df_arm <- df_arm %>% mutate(tp_label = factor(code2label[tp_code], levels = code2label[tp_codes]))
  ...
  ggplot(df_arm, aes(x = tp_label, y = diversity, fill = tp_label)) +
    ...
    geom_line(aes(group = patient_id), colour = "black") +
...
for (celltype in celltypes) {
  ...
  for (div_idx in diversity_indices) {
    ...
    df_merge <- raw_df %>% inner_join(patient_metadata_df %>% select(patient_id, Arm, group), by = "patient_id")
    if (nrow(df_merge) < 4) next
    ...
    for (pair_name in names(timepoint_pairs)) {
      ...
      pdf(file = file.path(out_dir, sprintf("%s_%s_%s_boxplot.pdf", pair_name, div_idx, celltype)), width = 12, height = 5)
      print(combined)
      dev.off()
    }
  }
